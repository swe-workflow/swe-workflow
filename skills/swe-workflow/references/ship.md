...
## Stage 7 — Close out
8. **Adversarial review — a fresh, read-only context, before the PR.** Have a *separate* context (a sub-agent or fresh session — **not** the implementer that wrote the code, and **not** `progress.md`'s own success narrative) read the **diff**, `task_plan.md` (AC checkboxes + Decisions), and the issue's acceptance criteria, and confirm the change actually meets each AC. It is **read-only — it reports gaps, it doesn't fix them** (separation of duties; only the implementer holds write access). Resolve a reported gap by looping back to Stage 6; a gap that's a genuine judgment call is bar-crossing — log it per `log-decisions` (a flagged assumption, or escalate/park if it needs you). Proceed only once it signs off. *Why a fresh context:* an agent grading its own summary is the most common unattended-run failure — verification must read actual state. See [Adversarial review](../REFERENCE.md#adversarial-review-before-close-out).
9. Open the PR with the body drawn from `progress.md` highlights — the session log IS the narrative; don't rewrite it. Include an **"Autonomy decisions"** section in the PR body, drawn from `DECISIONS.staged.md`, so the reviewer sees what was decided unattended — especially the flagged **assumptions** (`assumed`) to confirm.
10. After the PR merges, **promote the journal, then tear down** — **from the main checkout** (not inside the worktree): first append any `DECISIONS.staged.md` entries to repo-root `DECISIONS.md` and commit them as their own `log:` commit (serialized by teardown → conflict-free across parallel worktrees); then `git worktree remove ../<repo>-issue-<id>` (which discards the staging file), and delete the branch if it merged into the default branch.
