...
2. **Synthesize the PRD** — at the end of that conversation, invoke `to-spec`. It turns the grill conversation into **one PRD for this feature** (Problem / Solution / User Stories / Implementation + Testing Decisions / Out of Scope), published per the active tracker and labeled `ready-for-agent`. **One feature → one PRD.**
...
**Next:** slice the PRD into tracer-bullet issues with `to-tickets` (stage 4, separate), then `/ship` or `/ship-all`.
