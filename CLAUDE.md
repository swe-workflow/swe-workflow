...
- `skills/swe-workflow/` — the one **workflow skill** that conducts the whole 0→7 chain: `SKILL.md` (the map + conductor), `REFERENCE.md` (per-stage detail + the always-on rules registry, read when something feels off), `references/{setup,spec,to-features,grill-feature,ship,ship-all,status}.md` (the seven **stage procedures**; `ship.md` holds the single-source planner prompt, while `to-features.md` and `grill-feature.md` are the single-source stage-2 / step-3 procedures that `/spec` runs inline and the standalone `/swe-workflow:to-features` / `/grill-feature` commands also run), `trackers/` (tracker-adapter docs).
