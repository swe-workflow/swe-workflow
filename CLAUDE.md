...
This suite **orchestrates external skills it does not bundle** — `planning-with-files`, `tdd`, and mattpocock's `setup-matt-pocock-skills` / `grill-with-docs` / `to-spec` / `to-tickets` / `triage` — all themselves cross-agent Agent Skills. `swe-setup` auto-installs them. When editing, assume these are separate skills, not files in this repo.
