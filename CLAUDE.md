...
- `skills/swe-workflow/` — the one **workflow skill** that conducts the whole 0→7 chain, in **two layers**: `SKILL.md` (a pure **router** ≤100 lines — the stage table, conductor procedure, and invariants) and `references/{setup,spec,to-features,grill-feature,ship,ship-all,status}.md` (the seven **self-contained stage procedures**, each loaded only when its stage runs; `ship.md` holds the single-source planner prompt *and* all execution-layer detail — landing, teardown, stop conditions, adversarial review; `to-features.md` and `grill-feature.md` are the single-source stage-2 / stage-3 procedures that `/spec` runs inline and the standalone commands also run), plus `trackers/` (tracker-adapter docs). There is **no third detail layer** (`REFERENCE.md` was dissolved in 2.0.0) — a fact lives in the stage file where it bites.
- The `log-decisions` companion skill (the append-only decision journal) lives in its **own repo** — [`swe-workflow/log-decisions`](https://github.com/swe-workflow/log-decisions), extracted in 3.0.0 because it's general-purpose — and is orchestrated as an external skill (setup installs it). Stage-2 feature enumeration is **not** a separate skill — it's the `swe-workflow` skill's `references/to-features.md` procedure + the `/swe-workflow:to-features` command shim (a stage, per the no-split-stages rule below).
- `README.md` — install (universal `npx skills add` + Claude plugin), per-agent invocation, and a compact **name-free** flow visual (a deliberate, rename-immune duplicate of the chain *shape* only — the canonical map is SKILL.md's stage table).
- `docs/DESIGN.md` — the **design rationale** (philosophy, seam rule, spec-kit comparison, session topology, parallel execution, always-on-rules registry, further reading). Human-only — never in any agent's load path, and not distributed with the skill (`npx skills add` ships only `skills/`), so **skill files must never relative-link it**; use the GitHub URL if a pointer is unavoidable.
...
## The workflow's architecture (read SKILL.md for the map, docs/DESIGN.md for the rationale)
...
This suite **orchestrates external skills it does not bundle** — `log-decisions` (the suite's own, in its own repo), `planning-with-files`, `tdd`, and mattpocock's `setup-matt-pocock-skills` / `grill-with-docs` / `to-spec` / `to-tickets` / `triage` — all themselves cross-agent Agent Skills. `swe-setup` auto-installs them. When editing, assume these are separate skills, not files in this repo.
...
4. **Decision journal** (the external `log-decisions` skill). Bar-crossing build decisions append to per-worktree `DECISIONS.staged.md` (gitignored), promoted to repo-root `DECISIONS.md` at close-out. The decide/assume/escalate call is a 2×2 over *determinable × reversible*, with a catastrophic-action floor that always escalates.
...
- **One skill + many commands (the planning-with-files pattern).** Behavior lives in the single `swe-workflow` skill — a workflow stage is a `skills/swe-workflow/references/<stage>.md` procedure the conductor runs (`SKILL.md` is a pure router, keep it ≤100 lines; each stage file is **self-contained** — there is no separate detail layer to push things to). A `commands/<name>.md` is at most a thin Claude-only shim that delegates (`Invoke the swe-workflow skill and run its <stage> procedure…`) — **never put behavior there**, or non-Claude agents miss it. **Don't split stages into separate skills**: Claude always scans `skills/` with no allowlist, so extra skills can't be hidden there — Claude shows the one `swe-workflow` skill + the `/swe-workflow:*` commands, and other agents invoke the skill (it routes to the stage).
- **The seam rule (external skills).** The suite documents **only the seam** with each external skill it orchestrates: what it feeds it, the invocation, the artifact that must exist afterward, and any constraint the suite *adds*. Never restate the external's own interview flow, states, or output format — it documents itself. An external skill's name appears **only in the stage file that invokes it** (plus setup's install table and README's prerequisites), so an upstream rename is a one-file edit — the `/to-prd`→`/to-spec` rename once rippled through eight docs.
- **Single source of truth for the planner prompt.** The exact Stage-5 planner prompt lives **only** in the ship procedure (`skills/swe-workflow/references/ship.md`, Stage 5 step 6). `SKILL.md` and the `commands/ship.md` shim deliberately point at it rather than restate it, so it can't drift. Don't copy it elsewhere.
...
- **Don't double-track.** Each fact has one home (SKILL.md, Invariants). The same discipline applies to the docs themselves: cross-link rather than restate — the only sanctioned duplicate is README's name-free flow visual.
